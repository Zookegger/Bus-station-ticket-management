...
namespace Bus_Station_Ticket_Management.Areas.Admin.Controllers
{
    ...
    public class DriverController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("FullName,Email,PhoneNumber,DateOfBirth,Gender,Address,Licenses")] DriverViewModel model, IFormFile avatar, string password)
        {
            try
            {
                var validationResult = ValidateDriverModel(model, avatar);
                if (!validationResult)
                {
                    return View(model);
                }
        ...
        public async Task<IActionResult> Edit(string id, DriverViewModel model, IFormFile? Avatar, string? password)
        {
            ...
            var validationResult = ValidateDriverModel(model, Avatar, id);
            if (!validationResult)
            {
                return View(model);
            }
        ...
        private bool ValidateDriverModel(DriverViewModel model, IFormFile avatar, string? id = null)
        {
            if (model == null)
            {
                return false;
