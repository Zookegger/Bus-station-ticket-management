...
namespace Bus_Station_Ticket_Management.Areas.Admin.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        private bool ValidateEmployee(ApplicationUser applicationUser)
        {
            ...
            if (applicationUser != null && !string.IsNullOrEmpty(applicationUser?.PhoneNumber) && (applicationUser.PhoneNumber.Length < 8 || applicationUser.PhoneNumber.Length > 11))
            {
                ModelState.AddModelError("PhoneNumber", "Invalid phone number!");
                return false;
            }

            if (applicationUser != null && applicationUser.DateOfBirth.HasValue && (applicationUser.DateOfBirth.Value > DateOnly.FromDateTime(DateTime.Now)))
            {
                ModelState.AddModelError("DateOfBirth", "Date of birth cannot be in the future");
                return false;
            }

            if (applicationUser != null && applicationUser.DateOfBirth.HasValue && (DateTime.Now.Year - applicationUser.DateOfBirth.Value.Year) < 18)
            {
        ...
        private ViewResult ReloadViewWithRoles(ApplicationUser applicationUser, List<string> roles, ApplicationUser user)
        {
            ViewBag.Roles = roles;
